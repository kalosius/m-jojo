...
class Sale(models.Model):
    ...
    def save(self, *args, **kwargs):
        # When saving, update issued_quantity and total_quantity
        if self.pk is None:  # Only update quantities on first save (new sale)
            self.item.issued_quantity = F('issued_quantity') + self.quantity
            self.item.total_quantity = F('total_quantity') - self.quantity
            self.item.save(update_fields=['issued_quantity', 'total_quantity'])
